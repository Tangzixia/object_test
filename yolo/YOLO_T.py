...
参考：
    1、https://hk.saowen.com/a/40ab0e0202854d7becf89b73a0a1a54b2f254db9e72deb977ade6115c06898eb
    2、https://zhuanlan.zhihu.com/p/25053311

解析：
    对于x，y求得loss直接进行做差，而对于w，h求得loss却需要利用sqrt()进行求解，这样的原因在于
    YOLO对于小目标的检测效果非常差，因此更加侧重于小目标的loss～
'''
...
class YOLONet(net):
    ...
    def inference(self,images):
        ...
        :return: 4-D tensor [batch_size,cell_size,cell_size,num_classes+5*boxes_per_cell]
        '''
        conv_num=1
        ...
        temp_conv=self.conv2d('conv'+str(conv_num),temp_pool,[1,1,64,192],stride=1)
        conv_num+=1
        temp_pool=self.max_pool(temp_conv,[2,2],2)
        ...
        conv_num+=1
        temp_conv=self.conv2d('conv'+str(conv_num),temp_conv,[1,1,512,1024],stride=1)
        conv_num+=1
        temp_conv=self.max_pool(temp_conv,[2,2],2)
        ...
        local2=self.local('local2',local1,self.cell_size*self.cell_size*(self.num_classes+5*self.boxes_per_cell))
        local2=tf.reshape(local2,[tf.shape(local2)[0],self.cell_size,self.cell_size,self.num_class+self.boxes_per_cell*5])
        predicts=local2
    ...
    def iou(self,boxes1,boxes2):
        ...
        :param boxes1:4-D tensor[cell_size,cell_size,boxes_per_cell,4]
        :param boxes2:1-D tensor[4]===>(x_center,y_center,w,h)
        :return: iou:3-D tensor[cell_size,cell_size,boxes_per_cell]
        '''
    ...
    def cond1(self,num,object_num,loss,predict,labels,nilboy):
        '''
        if num<object_num
        :param num:
        :param object_num:
    ...
    def body1(self,num,object_num,loss,predict,labels,nilboy):
        ...
        :param object_num:
        :param loss:
        :param predict: 3-D tensor[cell_size,cell_size,5*boxes_per_cell]
        :param label: [max_objects,5] (x_center,y_center,w,h,class)
        :param nilboy:
        :return:
        参数解析：
        num: 第几张图片
        object_num:一张图片中能够有多少个目标
        loss:[class_loss, object_loss, noobject_loss, coord_loss]四个部分的loss
        predict：预测的坐标信息和置信度，和在各个类别上的概率分布
        labels：图片中目标的位置和对应的类别
        nilboy：网格是否包含目标
        ...
        # 注意这儿的max_y-min_y，max_x-min_x是合理的，因为需要计算类别损失，因此需要判断对应的物体包含那些框
